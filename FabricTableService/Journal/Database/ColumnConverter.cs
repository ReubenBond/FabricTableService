    ...
    internal class ColumnConverter
    {
        ...
        private static readonly IDictionary<Type, RetrieveColumnDelegate> RetrieveColumnDelegates =
            new Dictionary<Type, RetrieveColumnDelegate>
            {
                ...
                { typeof(string), (s, t, c) => Api.RetrieveColumnAsString(s, t, c, Encoding.Unicode) },
        ...
        public ColumnConverter(Type type)
        {
            ...
            if (!SetColumnDelegates.TryGetValue(type, out setColumnDelegate))
            {
            ...
            if (!RetrieveColumnDelegates.TryGetValue(type, out getColumnDelegate))
            {
                if (!IsSerializable(type))
                {
            ...
            JET_coltyp value;
            if (!Coltyps.TryGetValue(type, out value))
            {
                if (!IsSerializable(type))
                {
        ...
        private static bool IsSerializable(Type type)
        {
            ...
            if (typeof(Uri) == type || typeof(IPAddress) == type)
            {
                return true;
            }
